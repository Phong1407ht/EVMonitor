...
class StationDetailScreen(carContext: CarContext, private val placeId: String, private val viewModel: EvViewModel) : Screen(carContext) {
    init {
        ...
        paneBuilder.addAction(
            Action.Builder()
                ...
                .setOnClickListener {
                    ...
                    val lng = detail.geometry.location.lng
                    val uri = android.net.Uri.parse("geo:$lat,$lng?q=${android.net.Uri.encode(detail.name)}")
                    val intent = Intent(CarContext.ACTION_NAVIGATE, uri)
                    try {
                        carContext.startCarApp(intent)
                    } catch (e: Exception) {
                        Log.e("StationDetailScreen", "Could not start navigation", e)
                    }
                }
                .build()
        )

        return PaneTemplate.Builder(paneBuilder.build()).setTitle(detail.name).setHeaderAction(Action.BACK).build()
    }
}
